...
@MainActor
protocol GoogleCalendarTokenProviding: AnyObject {
...
@MainActor
final class GoogleCalendarOAuthService: GoogleCalendarTokenProviding {
    ...
    /// that originate from a background API call rather than from the manager.
    ...
    func clearTokens() {
        ...
        Defaults[.googleCalendarTokenExpiration] = 0
    }
    ...
    func validAccessToken(forceRefresh: Bool = false) async -> String? {
        if !forceRefresh,
           ...
           !cachedToken.isEmpty,
           Defaults[.googleCalendarTokenExpiration] > Date().timeIntervalSince1970 + Self.expiryLeeway {
            return cachedToken
        }
    ...
    private func exchangeToken(body: [String: String], clientSecret: String, grant: Grant) async throws {
        ...
        // always has the secret available, without duplicating it in Defaults.
        tokenStore.write(clientSecret, account: .clientSecret)
        Defaults[.googleCalendarTokenExpiration] = Date().timeIntervalSince1970 + token.expiresIn
        onTokenStateChange?()
    }
    ...
    /// pair the app would keep reporting itself connected while every fetch
