...
///
/// Unlike Spotify's login (a custom `atoll-spotify://` scheme intercepted by
/// ASWebAuthenticationSession), Google's "Desktop app" OAuth client type
/// requires a loopback redirect per RFC 8252 -- and Google's own sign-in
/// pages reject requests originating from an embedded WebView with
/// `Error 403: disallowed_useragent`, which rules out
/// ASWebAuthenticationSession (WebKit-backed) entirely. The browser step
...
final class GoogleCalendarLoopbackServer {
    ...
    private let queue = DispatchQueue(label: "com.Ebullioscopic.Atoll.GoogleCalendarLoopback")
    ...
    private func respond(on connection: NWConnection, success: Bool) {
        ...
        let message = success
            ? "Google Calendar is connected. You can close this tab and return to Atoll."
            : "Atoll did not receive an authorization code. You can close this tab and try again."
        let body = """
